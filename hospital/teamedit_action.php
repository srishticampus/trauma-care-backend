...
if($_FILES["file"]["name"]==""){
  $sq="select * from doctors where id=$doctor";
  $res=$con->query($sq);
  $ro=$res->fetch_assoc();
  $image=$ro['file'];
}
 ...
 $timeslot7=$_POST['timeslot7'];
            ...
            $sql="UPDATE `doctors` SET `hospital_id`='$id',`doctor_name`='$name',`doctor_email`='$email',`phone`='$phone',`department`='$department',`degree`='$degree',`designation`='$designation',`experience`='$experience',`file`='$image',`payment`='$fee' WHERE id=$doctor";
            $result=$con->query($sql);
            
        
             $last_id = $doctor;
                 $q1="delete from doc_days where doc_id=$last_id";
        $rq=$con->query($q1);
        $q2="delete from doctor_timeslot where doc_id=$last_id";
        $rq2=$con->query($q2);
